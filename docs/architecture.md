...
- Models × Benchmarks matrix; grid columns per model, global benchmark filter, per-row alignment by filename/prompt.
...
- `Benchmark`: id (uuid), name (unique), slug, created_at.
- `ImageArtifact`: id (uuid), model_id FK, benchmark_id FK, source_url, prompt, prompt_hash, filename, dimensions, metadata JSONB, captured_at, created_at. Unique index on (model_id, benchmark_id, filename) to align rows.
- `ViewKV`: context (text), key (text), value (jsonb), created_at, updated_at. Composite PK `(context, key)`.
- `ModelTag` (optional, for future filters), `BenchmarkTag` possible extension.
...
- Routes:
  - `GET /api/models?search=&limit=...` → latest-first list, includes benchmark counts.
  - `GET /api/benchmarks?search=...` → latest-first list.
  - `GET /api/images?modelIds=&benchmarkId=&cursor=&take=` → paginated artifacts for selected columns.
  - `POST /api/ingest` → called by Python package; payload includes model, benchmark, images array. Idempotent on (model, benchmark, filename); upsert metadata.
...
- Provides:
  ...
  - `register_view(model_name, benchmark_name, images: List[ImageSpec])`.
  - CLI: `eval-board ingest --model <name> --benchmark <name> --from-s3 s3://... --config config.yml`.
...
- Global context `ViewContext` storing current grid spec (models array, benchmark filter, sort order).
...
- Apply indexes on `(model_id, benchmark_id, filename)` and `created_at` fields.
...
- Need for prompt grouping/filters beyond benchmarks?
