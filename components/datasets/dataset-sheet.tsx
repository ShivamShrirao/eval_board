...
export function DatasetSheet({ datasetId, datasetName, onClose }: DatasetSheetProps) {
  return (
    <Dialog.Root open={Boolean(datasetId)} onOpenChange={(open) => !open && onClose()}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed inset-0 z-50 flex flex-col overflow-hidden border-l border-slate-900 bg-black/95 shadow-2xl sm:left-auto sm:w-[680px]">
          ...
          <div className="flex-1 overflow-auto px-6 py-6">
            {datasetId ? <DatasetGallery datasetId={datasetId} /> : null}
          </div>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
